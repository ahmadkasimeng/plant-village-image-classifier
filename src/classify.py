...
if len(config_input) > 0 and config_input[-1].isdigit():
    ...
    print(
        f"🔢 Data Limit Set to: {'ALL' if data_limit is None else data_limit}")
...
if len(config_input) == 0 or config_input == ['BASIC']:
    ...
    # Custom Mode
    if config_input[0] == '18':
        model_type = "RESNET18"
    elif config_input[0] == '50':
        model_type = "RESNET50"

    if config_input[1] == 'RF':
        clf_type = "RF"
    elif config_input[1] == 'GB':
        clf_type = "GB"
    ...
    print(
        f"⚠ Unrecognized configuration: {args.config}. Defaulting to Basic.")
...
if __name__ == "__main__":
    ...
    print(
        f"⏱  Feature Extraction took: {timings[f'Feature Extraction ({model_type})']:.2f}s")
    ...
    print(
        f"⏱  Training took: {timings[f'Classifier Training ({clf_type})']:.2f}s")
    ...
    try:
        ...
        with open(log_file_path, "a", encoding="utf-8") as f:
            ...
            f.write(
                f"Dataset Limit: {'ALL' if data_limit is None else data_limit}\n")
